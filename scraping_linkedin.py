...
def scrape_Job_details(num_jobs ):
    ...
    list_job_IDs = list_job_IDs[0:num_jobs]
...
def scraping_main(
    ...
    jobs_DF = scrape_Job_details(num_jobs = num_jobs)
   ...
   # jobs_DF = pd.read_json(
    ...
    jobs_DF.to_csv("../data/linkedin_jobs_scraped.csv", index = False )
...
def main():
    if len(sys.argv) == 5:
        keywords, location, sleep_time_, num_jobs_ = sys.argv[1:]
        sleep_time_ = int(sleep_time_)
        num_jobs_ = int(num_jobs_)
        scraping_main(keywords, location, sleep_time=sleep_time_, num_jobs = num_jobs_)
    else:
        print(
